  ...
  return (
    <div className="min-h-screen bg-[color:var(--bvt-bg)] text-[color:var(--bvt-ink-body)] font-sans dark">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-16 items-start">
            <div className="lg:col-span-8">
              <h1 className="font-display text-[color:var(--bvt-ink)] leading-[0.98] tracking-[-0.02em] text-[44px] sm:text-[56px] md:text-[72px] lg:text-[88px]">
                Bali villa ROI audits.
                <br />
                <span className="text-[color:var(--bvt-accent)]">Stress-tested net yields.</span>
              </h1>
      ...
      <div className="max-w-[1400px] mx-auto mb-10 md:mb-14">
        <div className="flex items-center gap-3 mb-6">
          <span className="h-px w-10 bg-[color:var(--bvt-accent)]" aria-hidden />
          <span className="label-micro">The ledger · {loading ? '…' : auditedCount.toLocaleString()} audited dossiers</span>
        </div>
           ...
           <p className="hidden md:flex items-center gap-2">
             <span className="label-micro">
               {loading
                 ? 'Loading dossiers'
                 : filtersActive
                   ? `Showing ${processedListings.length.toLocaleString()} of ${auditedCount.toLocaleString()} matching`
                   : `Top ${displayListings.length.toLocaleString()} of ${auditedCount.toLocaleString()} audited · Filter to see all`}
             </span>
           </p>
           <button
      ...
      <main className={`${mobileView === 'list' ? 'block' : 'hidden'} md:hidden transition-all w-full`}>
        ...
        {!loading && !filtersActive && auditedCount > displayListings.length && (
          <div className="mt-5 border-t border-b border-[color:var(--bvt-hairline)] py-5 px-4 text-center">
            <p className="label-micro !text-[color:var(--bvt-ink-muted)]">
              Showing the top <span className="font-mono tabular-nums text-[color:var(--bvt-ink)]">{displayListings.length}</span> of <span className="font-mono tabular-nums text-[color:var(--bvt-ink)]">{auditedCount.toLocaleString()}</span> audited dossiers.
            </p>
