...
export default function WebsiteDashboardLogin() {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch("/api/website-dashboard/login", {
        method: "POST",
  ...
  return (
    <main className="min-h-screen bg-[color:var(--bvt-bg)] text-[color:var(--bvt-ink-body)] flex items-center justify-center px-6 py-12">
      <section className="w-full max-w-[460px] border border-[color:var(--bvt-hairline)] bg-[color:var(--bvt-bg-elev)] rounded-md p-6 md:p-8">
        ...
        <h1 className="font-display text-[34px] leading-tight tracking-[-0.02em] text-[color:var(--bvt-ink)]">
          Website dashboard locked
        </h1>
        <p className="mt-4 text-[14px] leading-relaxed text-[color:var(--bvt-ink-muted)]">
          This private dashboard tracks website improvement work, known blockers,
          live checks, and next actions for Bali Villa Truth.
        </p>

        <form onSubmit={submit} className="mt-7 space-y-4">
          <div>
            <label htmlFor="website-dashboard-password" className="label-micro mb-2 block">
              Password
            </label>
            <input
              id="website-dashboard-password"
              type="password"
              value={password}
